...
#
# Usage:  cd /path/to/labsmith && bash scripts/labsmith-wizard.sh
#         (Legacy bash wizard; primary UI is bash labsmith.sh → Textual TUI.)
...
# Repo root: this file lives in scripts/ or (legacy) at repo root
if [ -f "$SCRIPT_DIR/../chunker.py" ]; then
    LABSMITH_DIR="$(cd "$SCRIPT_DIR/.." && pwd)"
else
...
. "$LABSMITH_DIR/scripts/prereqs-common.sh"
